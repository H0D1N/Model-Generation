...
class PromptBasedModel(nn.Module):
    def __init__(self,task_embed=True,ada_kernel=True,is_embed=True,attn_pad=False,vocab_size=2,kernel_number=2048,
                 block=Bottleneck,layers=[3,4,6,3],classifer_num=100):
        ...
        self.backbone=ada_ResNet(block,layers,classfier_num=classifer_num)
    def forward(self,prompt,img):
        if self.ada_kernel:
            selection,act_selection=self.gate_network(prompt)
            logits=self.backbone(img,selection,act_selection)
